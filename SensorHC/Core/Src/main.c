...
#define MOTOR_RIGHT_CALIBRATION  0.95
...
int main(void)
{
  ...
  // 1. Khoi dong Timer delay va Ngat Input Capture cho cam bien sieu am
  HAL_TIM_Base_Start(&htim1);
  HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_1);

  // 2. Khoi dong ca 2 kenh xung PWM dieu khien L298N
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_1); // Chan ENA (Banh trai)
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_2); // Chan ENB (Banh phai)

  // 3. Cau hinh huong quay TIEN mac dinh ban dau cho ca 2 banh
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_RESET);// IN4 = 0
  ...
  // Khai bao cac moc thoi gian chay da nhiem khong ham delay
  ...
  while (1)
  {
      ...
      float motor_right_calibration = 0.95f;
      ...
      if (current_tick - last_motor_tick >= 50)
      {
          ...
          // Kiem tra ngoai gioi han do cua cam bien (Out of range)
          if (Distance_cm < 2 || Distance_cm > 400)
          {
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // Tat LED
              HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); // Tat Buzzer
              current_pwm = 0; // Tat dong co cho an toan
          }
          // DANGER ZONE: < 50 cm
          else if (Distance_cm < 50)
          {
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);   // Bat LED
              HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET);   // Buzzer keu
              current_pwm = 0; // Dung xe
          }
          // WARNING ZONE: 50 cm - 100 cm
          else if (Distance_cm <= 100)
          {
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);   // Bat LED
              HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); // Khong keu Buzzer
              current_pwm = 500; // Giam toc do chay cham
          }
          // SAFE ZONE: > 100 cm
          else
          {
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // Tat LED
              HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); // Tat Buzzer
              current_pwm = 600; // Toc do binh thuong
          }

          // --- THUC HIEN CAN BANG TOC DO 2 BANH TAI DAY ---
          ...
          __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, pwm_left);  // Banh trai
          __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, pwm_right); // Banh phai
      }
      ...
      if (current_tick - last_lcd_tick >= 200)
      {
          ...
          // --- DONG 2: Hien thi Trang thai (Status) ---
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM2)
    {
        ...
        else if (Is_First_Captured == 1)
        {
            ...
            {
                Difference = (__HAL_TIM_GET_AUTORELOAD(htim) - IC_Val1) + IC_Val2;
            }
